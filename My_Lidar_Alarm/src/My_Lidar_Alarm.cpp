  ...
  float ping_dist_sweep_=3.0; // global var to hold length of a SINGLE LIDAR ping--in front
  ...
  void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
  	...
  	for(int i = ping_index_ - 200; i < ping_index_ + 200; i++){ 
  		ping_dist_sweep_ = laser_scan.ranges[i];

  		ROS_INFO("ping dist in front = %f",ping_dist_sweep_);
  		if (ping_dist_sweep_<MIN_SAFE_DISTANCE) {
  	...
  	lidar_dist_msg.data = ping_dist_sweep_;
