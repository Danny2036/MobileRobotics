...
// the action message can be found in: .../Gazebo_Actions/action/demo.action
// automated header generation creates multiple headers for message I/O
// these are referred to by the root name (demo) and appended name (Action)
// If you write a new client of the server in this package, you will need to include Gazebo_Actions in your package.xml,
// and include the header file below
#include<Gazebo_Actions/gazebo.action.h>


// This function will be called once when the goal completes
// this is optional, but it is a convenient way to get access to the "result" message sent by the server
void doneCb(const actionlib::SimpleClientGoalState& state,
    const Gazebo_Actions::demoResultConstPtr& result) {
...
int main(int argc, char** argv) {
        ...
        int g_count = 0;
        // here is a "goal" object compatible with the server, as defined in Gazebo_Actions/action
        Gazebo_Actions::demoGoal goal; 
        
        // use the name of our server, which is: example_action (named in Gazebo_Actions.cpp)
        // the "true" argument says that we want our new client to run as a separate thread (a good idea)
        actionlib::SimpleActionClient<Gazebo_Actions::gazebo.action> action_client("example_action", true);
